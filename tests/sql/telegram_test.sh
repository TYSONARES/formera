#!/usr/bin/env bash
# 0018 Telegram bildirim trigger'ini bos bir Postgres'te dogrular.
# pg_net + vault shimlenir; gercek Telegram cagrisi yapilmaz.
set -euo pipefail
PORT="${PGPORT:-55432}"; HOST="${PGHOST:-/tmp}"; DB=formera_tg_test
...
echo "senaryolar:"
psql -h "$HOST" -p "$PORT" -U postgres -d "$DB" -t -A -f "$HERE/telegram_notify.sql" 2>&1 | sed 's/^/  /'
